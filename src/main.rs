...
use anyhow::Context;
use std_versions::VersionConstructor;

mod std_versions;

struct VersionAnalyzer<'a> {
    version_constructor: &'a VersionConstructor,

    path: Vec<String>,
    nested_unsafe: usize,

    version_counts: HashMap<String, usize>,
    total_exprs: usize,
    unsafe_exprs: usize,
}
...
impl<'a> VersionAnalyzer<'a> {
    ...
    fn process_file(&mut self, file: syn::File) {
...
fn main() -> anyhow::Result<()> {
    let version_constructor =
        std_versions::load_version_constructor().context("could not process std versions")?;

    let file: syn::File = syn::parse_str(include_str!("../expanded-tokio.rs"))
        .context("could not parse crate source code")?;
